...
|------|------------|-------------|
| Бэкенд | Python 3.11+, Django 5.x | REST API, бизнес-логика |
| База данных | PostgreSQL 15+ | Хранение данных, транзакции |
| Фронтенд | React 18 (SPA) | Отдельное приложение, взаимодействие через REST |
| Аутентификация | JWT + Session Auth | Логин/пароль, роли |
| Историзация | `django-simple-history` | Аудит изменений критичных сущностей |
...
- Монолитный бэкенд с чёткими модульными границами.
...
- Вход по логину и паролю (JWT + Session Auth).
...
| **Историзация критичных сущностей** | Изменения `PlotOwnership`, `Accrual`, `Payment`, `Expense` фиксируются автоматически (`django-simple-history`). |
...
| История критичных изменений | `django-simple-history` для `PlotOwnership`, `Accrual`, `Payment`, `Expense` |
