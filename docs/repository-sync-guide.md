...
## Правила работы с руководством

- **Агент (AI)** перед выполнением шагов из этого руководства: запрашивает разрешение, кратко описывает, что собирается сделать, и выполняет действия только после вашего **чёткого подтверждения** (например: «да», «выполняй», «подтверждаю»).
- Без вашего подтверждения выполняться не должно.
