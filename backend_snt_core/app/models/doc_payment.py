...
class DocPayment(Base):
    __tablename__ = "doc_payment"
    __table_args__ = (
        UniqueConstraint("snt_id", "number", name="uq_doc_payment_snt_number"),
    )
...
class DocPaymentRow(Base):
    __tablename__ = "doc_payment_row"
    __table_args__ = (
        UniqueConstraint("snt_id", "id", "doc_id", name="uq_doc_payment_row_id_doc"),
        ForeignKeyConstraint(
            ["snt_id", "doc_id"],
            ["doc_payment.snt_id", "doc_payment.id"],
            name="fk_doc_payment_row_doc",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["snt_id", "plot_id"],
            ["plot.snt_id", "plot.id"],
            name="fk_doc_payment_row_plot",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["snt_id", "owner_id"],
            ["owner.snt_id", "owner.id"],
            name="fk_doc_payment_row_owner",
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["snt_id", "charge_item_id"],
            ["charge_item.snt_id", "charge_item.id"],
            name="fk_doc_payment_row_charge_item",
            ondelete="RESTRICT",
        ),
        CheckConstraint("amount > 0", name="chk_doc_payment_row_amount_positive"),
    )
